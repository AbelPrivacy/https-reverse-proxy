...
int main() {
    ...
    if (wolfSSL_CTX_use_certificate_file(ctx, "cert.pem", WOLFSSL_FILETYPE_PEM) != WOLFSSL_SUCCESS)
        err_sys("Failed to load cert");

    if (wolfSSL_CTX_use_PrivateKey_file(ctx, "key.pem", WOLFSSL_FILETYPE_PEM) != WOLFSSL_SUCCESS)
